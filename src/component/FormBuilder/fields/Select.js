...
class FbFieldSelect extends React.Component {
  constructor(props) {
    super(props);
    // create refs

    try {
      this.sug = this.props.options || ["NO DATA"];
    } catch (error) {
  ...
  handleOptionClick = (e) => {
    e.preventDefault();
    this.setState({ value: e.target.innerText, show: false });
  };
  ...
  render() {
    return (
      <React.Fragment>
        ...
        <img
          src={down}
          className="fb-select-down"
          onMouseDown={this.onDownClick}
          ref={this.dropDown}
        />
        {this.state.show ? (
          <ul className="fb-select" ref={this.list}>
...
export default FieldHOC(FbFieldSelect);
